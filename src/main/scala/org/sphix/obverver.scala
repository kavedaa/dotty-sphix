...
class ChangeObserver[A, U](observableValues: Seq[ObservableValue[A]], f: (ObservableValue[? <: A], A, A) => U)
  extends ChangeObserverLike(observableValues):
    def changed(ov: ObservableValue[? <: A], oldValue: A, newValue: A) = f(ov, oldValue, newValue)