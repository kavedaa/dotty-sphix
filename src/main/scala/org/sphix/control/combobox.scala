...
import org.sphix.control.cell.To

trait ComboBoxUtils[T] extends ComboBox[T] with ListCells[T]:
  def setCell(listCell: => ListCell[T]) =
    setCellFactory(_ => listCell)

trait StringComboBoxUtils[T](render: T => String) extends ComboBoxUtils[T] with ListCells[T]:

  //  don't know why this is needed for empty comboboxes, it's a JavaFX thing
  private val nullSafeRender: To[T, String] = (x: T) => if x == null then "" else render(x)
